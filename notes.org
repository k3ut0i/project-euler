...
** TODO 66 - Diophantine Equation                            :pells_equation:
...
** TODO 77 - Prime Summations                                     :partition:
...
\end{gather}
The above equation is taken from math.stackexchange site, that discusses prime partitions. I'm not sure how it was derived. If a(n, k) is the number of prime summations of n, using only primes below k, then:
\begin{equation}
a(n, k) = 
\begin{cases}
  a(n, k-1) & \text{if } k \not\in \mathbb{P} \\
  \sum_{i=1}^{n\div k}a(n-i\cdot k, k-1) & \text{if } k \in \mathbb{P}
\end{cases}
\end{equation}

** TODO 78 - Coin Partitions                                      :partition:
...
** TODO 100 - Arranged Probability                           :pells_equation:
